...
export class McapViewerProvider implements vscode.CustomReadonlyEditorProvider<McapDocument> {
  ...
  private buildInit(document: McapDocument, phase: "preview" | "full"): InitMessage {
    ...
    return {
      ...
      stateTimestampsNs: summary.stateTimestampsNs.map((ts) => ts.toString()),
      actionTimestampsNs: summary.actionTimestampsNs.map((ts) => ts.toString()),
      ...
      seriesStartNs: summary.seriesStartNs?.toString(),
      seriesEndNs: summary.seriesEndNs?.toString(),
